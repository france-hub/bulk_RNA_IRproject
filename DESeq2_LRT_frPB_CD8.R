...
#DDS object
pb_cd8_fr_dds <- deseq_from_tibble(cd8_pb_fr_count, cd8_pb_fr_sample, design = ~ response)

#Create DESeq object
pb_cd8_fr_LRT <- DESeq(pb_cd8_fr_dds, test="LRT", reduced = ~ 1)

#Stailizing variance
rld_frpb_cd8_LRT <- rlog(pb_cd8_fr_LRT)

#Results
res_frpb_cd8_LRT <- results(pb_cd8_fr_LRT)

##Filter only genes which are significant
padj.cutoff <-  0.05
res_LRT <- res_frpb_cd8_LRT %>%
  data.frame() %>%
...
x <- top_counts(res_LRT, rld_frpb_cd8_LRT, sort_fc = FALSE, filter = FALSE, padj=0.05)
